...
if __name__ == "__main__":
    ...
    cols += [F.col('candidate.jdstarthist'), F.col('cdsxmatch'), F.col('candidate.ndethist')]

    pdf = df.select(cols).toPandas()

    with EmissionsTracker(tracking_mode='process', pue=1.25) as tracker:
        t0 = time.time()
        out = knscore.__wrapped__(*[pdf[col] for col in pdf.columns])
