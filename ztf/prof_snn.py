...
if __name__ == "__main__":
    ...
    cols = ['candid', 'cjd', 'cfid', 'cmagpsf', 'csigmapsf']
    cols += [F.col('roid'), F.col('cdsxmatch'), F.col('candidate.jdstarthist')]
    cols += [F.lit('snn_snia_vs_nonia')]

    pdf = df.select(cols).toPandas()

    with EmissionsTracker(tracking_mode='process', pue=1.25) as tracker:
        t0 = time.time()
        out = snn_ia.__wrapped__(*[pdf[col] for col in pdf.columns])
