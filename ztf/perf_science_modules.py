...
def save_on_disk(modules, total_memory, gb_per_executor, core_per_executor, night, total_alerts):
    """ """
    modules_ = {k: {} for k in modules.keys()}
    for kext in modules_.keys():
        modules_[kext] = {kinner: v for kinner, v in modules[kext].items() if kinner not in ['processor', 'cols']}

    modules_['config'] = "total_memory={}, gb_per_executor={}, core_per_executor={}, night={}, total_alerts={}".format(
